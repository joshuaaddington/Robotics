...
def compute_robot_path(arm, q_init, goal, obst_location, obst_radius):
      ...
      def get_error(q):
            end_effector_position = arm.fk(q)[:3,3]
            e = goal - end_effector_position
            return e
      error = get_error(q_init)
      count = 0

      while (np.linalg.norm(error) > 1e-4 and count < 1000):
            q = arm.ik_position(goal, obstacle = obst_location, obstacle_radius = obst_radius, q0 = q_init, method = "pinv", K = np.eye(3)/10,)
            error = get_error(q)
            count = count + 1
      q_s.append(q)
...
if __name__ == "__main__":
      ...
      q_ik_slns = compute_robot_path(midterm_arm, q_0, goal, obst_position, obst_rad)
