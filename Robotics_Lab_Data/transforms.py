...
def sym_se3(R=np.eye(3), p=np.array([0, 0, 0])):
    """
        T = se3(R, p)
        Description:
            Given a numpy 3x3 array for R, and a 1x3 or 3x1 array for p, 
    ...
    """
    # TODO - fill out "T"
    T = sp.Matrix([[R[0,0], R[0,1], R[0,2], p[0]],
                  [R[1,0], R[1,1], R[1,2], p[1]],
                  [R[2,0], R[2,1], R[2,2], p[2]],
                  [0, 0, 0, 1]])
...
def dh(th,d,a,alpha):
    T = np.array([[np.cos(th), -np.sin(th)*np.cos(alpha), np.sin(alpha)*np.sin(th), a*np.cos(th)],
                  [np.sin(th), np.cos(alpha)*np.cos(th), -np.sin(alpha)*np.cos(th), a*np.sin(th)],
                  [0, np.sin(alpha), np.cos(alpha), d],
                  [0, 0, 0, 1]])
    return T

def sym_dh(th,d,a,alpha):
    T = sp.Matrix([[sp.cos(th), -sp.sin(th)*sp.cos(alpha), sp.sin(alpha)*sp.sin(th), a*sp.cos(th)],
                  [sp.sin(th), sp.cos(alpha)*sp.cos(th), -sp.sin(alpha)*sp.cos(th), a*sp.sin(th)],
...
def R2axis(R):
    ...
    ang = np.acos((R[0,0]+R[1,1]+R[2,2]-1)/2) # TODO - fill out here. 
...
def axis2R(ang, v):
    ...
    """
    # TODO fill this out 
    R = np.array([[v[0]**2*(1-np.cos(ang))+np.cos(ang), v[0]*v[1]*(1-cos(ang))-v[2]*np.sin(ang), v[0]*v[2]*(1-np.cos(ang))+v[1]*np.sin(ang)],
...
def R2quat(R):
    ...
    return np.array([.5*np.sqrt(np.abs(R[0,0]+R[1,1]+R[2,2]+1)),
                     .5*np.sign(R[2,1]-R[1,2])*np.sqrt(np.abs(R[0,0]-R[1,1]-R[2,2]+1)),
                     .5*np.sign(R[0,2]-R[2,0])*np.sqrt(np.abs(R[1,1]-R[2,2]-R[0,0]+1)),
                     .5*np.sign(R[1,0]-R[0,1])*np.sqrt(np.abs(R[2,2]-R[0,0]-R[1,1]+1))])
